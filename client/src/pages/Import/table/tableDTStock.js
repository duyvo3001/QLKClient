...
const TBodytable = (props) => {
  ...
  const [DataMalk, setDataMalk] = useState(null)
  const { filters, valuehidden, searchBox } = props;
  //request render table

  useEffect(() => {
    // check if checkbox is !== 0 and !== undefined
  ...
  const HandleChange = (event) => {
    // Render when input in table is changed
    const { name, value } = event.target;
    const _id = _idItem;
    setFormData({ ...formData, [name]: value, _id });
  };
  if(DataMalk !== null){
    return <Navigate to={"/DetailProduct/"+DataMalk}/>; 
  }
    ...
    Data?.data?.result !== undefined
      ? Data?.data?.result?.map((key) => (
          <tr
            ...
            name="MaLK"
            value={key.MaLK}
            onClick={() => HandelClickTR(key.MaLK)}
          >
            <td>
              <div>{key.MaLK} </div>
            </td>
